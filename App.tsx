...
const App: React.FC = () => {
  ...
  const handleAddOrder = useCallback(async (order: Order) => {
    setAppState(prev => ({ ...prev, orders: [...(prev.orders || []), order] }));
    try {
      await upsertLocalEntity('orders', order);
      await queueUpsert('orders', order);
      setSyncKick((v) => v + 1);
    } catch (e) {
  ...
  const handleDeleteOrder = useCallback(async (id: string) => {
    setAppState(prev => ({ ...prev, orders: (prev.orders || []).filter(o => o.id !== id) }));
    try {
      await deleteLocalEntity('orders', id);
      await queueDelete('orders', id);
      setSyncKick((v) => v + 1);
    } catch (e) {
  ...
  const handleGenerateInvoice = useCallback((order: Order) => {
    // Placeholder - InvoiceGenerator component handles PDF generation
  ...
  return (
    <Layout currentView={currentView} onNavigate={setCurrentView} onLogout={() => setAuthStatus('landing')} alertCount={alertCount} appState={appState}>
      ...
      >
        <Suspense fallback={<ViewLoading />}><div key={currentView}>{renderView()}</div></Suspense>
      </ErrorBoundary>
    </Layout>
  );
};
